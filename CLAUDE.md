...
**abnormalia.com** is a Jekyll static site published on GitHub Pages, proxied through Cloudflare. It hosts *Ixens*: self-contained mixed-media minisites generated by a swamp workflow.
...
- `_layouts/default.html` — the only layout. All Jekyll pages (`index.html`, `ixen/index.html`, `privacy.md`, `cookies.md`) use it.
- `_includes/` — `head.html`, `header.html`, `footer.html` partials.
- `assets/css/site.css` — single stylesheet. No preprocessor, no framework.
- `_data/ixens.yml` — the Ixen registry (slug, title, date, description). Drives both the home page "Latest Ixens" list and the archive at `/ixen/`.

### Ixen pages

Individual Ixens live at `ixen/<slug>/index.html`. These are **standalone HTML files** — not processed by Jekyll and not using the site layout. They are entirely self-contained (inline CSS, own audio player, all media co-located).

The original I, Xen (2005) lives at `i-xen/` under the same convention.

### AI generation workflow

New Ixens are produced by the `generate-ixens` swamp workflow:

1. Edit `workflows/generate-ixens-input.yaml` — add the Ixen spec(s) to the `ixens` array and set generation parameters (`ixenModel`, `imageStyle`, `musicGenre`, etc.).
2. Add the entry to `_data/ixens.yml` so the Jekyll site registers it.
3. Run the workflow:
   ```bash
   swamp workflow run generate-ixens --input-file workflows/generate-ixens-input.yaml
   ```

**Workflow job order:** `prepare` → (`images` ∥ `music`) → `page`

- `prepare` — versions the existing output into a numeric subdirectory (`ixen/<slug>/1/`, `2/`, …).
- `images` — generates `hero.png` and `concept-N.png` files via `@alvagante/content-image` (OpenAI).
- `music` — generates or reuses `track.wav` via `@abnormalia/ixen-soundtrack` (Suno/Lyria).
- `page` — generates `ixen/<slug>/index.html` via `@alvagante/content-ixen` (Claude).

**Vaults required:** `anthropic-keys` (`api-key`), `openai-keys` (`api-key`), `onemin-keys` (`API_KEY`).
